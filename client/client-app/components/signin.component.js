;(function() {
  ...
  angular.module('app').component('signIn', {
    ...
    controller: function($location, authService, Notification, $routeParams) {
      ...
      vm.forgotPassword =
        $routeParams.action === 'forgotpassword' ? true : false
      ...
      var errorAlert = function(message) {
        ...
        if (message.errmsg && re1.test(message.errmsg)) {
          Notification.error(
            `The username '${
              message.op.username
            }' already exists please register with a different username`
          )
        }
        if (message.errmsg && re2.test(message.errmsg)) {
          Notification.error(
            `the e-mail '${message.op.email}' is already registered.`
          )
        }
        if (message.errmsg && re3.test(message.errmsg)) {
          Notification.error(
            `The phone number '${
              message.op.phoneNumber
      ...
      vm.authorisation = function() {
        ...
        if (vm.action === 'login') {
          //check the action type i.e whether login register reset password or forgot password.
          credentials = {
            //set the credentails.
        ...
        authService
          .register(url, credentials, headers)
